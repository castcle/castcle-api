...
const AccountDeviceSchema = new Schema({
  platform: SchemaTypes.String,
  uuid: SchemaTypes.String,
  firebaseToken: SchemaTypes.String,
});
...
class MigrateDevice {
  static run = async () => {
    ...
    await connect(url);
    const accountReferralModel = model('AccountDevice', AccountDeviceSchema);
    const accountModel = model('Account', AccountSchema);
    let limit = 1000;
    let offset = 0;

    while (true) {
      const devices = await accountReferralModel.aggregate([
        {
          $group: {
            _id: '$account',
            prepare: {
      ...
      const $devices = devices.map(async (item) => {
        const updateCount = accountModel.updateOne(
          { _id: item._id },
          {
            $set: {
              devices: item.prepare,
            },
          }
        );
        Logger.log(
...
MigrateDevice.run().catch(console.error);
