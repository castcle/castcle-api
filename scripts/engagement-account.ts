...
class Migrate {
  static run = async () => {
    ...
    await connect(url);
    const engagementModel = model('Engagement', EngagementSchema);
    let limit = 1000;
    let offset = 0;
    ...
    while (true) {
      ...
      const $engagements = engagements.map(async (item) => {
        const updateBy = await engagementModel.updateOne(
          { _id: item._id },
          { $set: { account: item.users.ownerAccount } },
        );
        Logger.log(
          JSON.stringify({ accountId: item.user, affected: updateBy.n }),
        );
      });
...
Migrate.run().catch(console.error);
