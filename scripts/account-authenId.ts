...
const AccountAuthenIdSchema = new Schema({
  ...
  socialId: SchemaTypes.String,
  socialToken: SchemaTypes.String,
  socialSecretToken: SchemaTypes.String,
...
class MigrateAuthenId {
  static run = async () => {
    ...
    while (true) {
      const authenIds = await accountReferralModel.aggregate([
        {
          $group: { _id: '$account', socials: { $push: '$$ROOT' } },
        },
        {
          $limit: limit,
        },
      ...
      if (authenIds.length === 0) break;

      console.time(JSON.stringify({ offset }));
      const $authenIds = authenIds.map(async (item) => {
        if (item.socials.length) {
          let prepare: any = {};
          item.socials.map((social: any) => {
            prepare[social.type] = {
              socialId: social.socialId,
              socialToken: social.socialToken || null,
...
MigrateAuthenId.run().catch(console.error);
